...
Client: FHM Hotelservice GmbH (Frankfurt am Main, Germany)Vendor: ZiroveMarket: Germany onlyPurpose: Exhaustive ledger of every decision confirmed across the entire requirements engagement. Nothing summarized, nothing dropped. This is the companion to the Pivot Design Document — the design doc says how we build, this register says everything we agreed.
...
🔵 The contract filled at signup is the SAME document as the one issued after probation (one document, not two). If it ever needs to differ, that change happens later.

✅ Contract is signed using a real e-signature with QES (Qualified Electronic Signature)

🔵 Recommended QES provider: Skribble (native QES, EU/German hosting, API-embeddable). Alternatives noted: Yousign, sproof.

🔵 QES is legally required because fixed-term employment contracts in Germany (§14(4) TzBfG) require it; a simple e-signature would invalidate the fixed term

[OPEN] Probation's legal shape (fixed-term trial contract vs. permanent contract with probation clause) — determines whether QES is mandatory from day one. Client: "to be answered later."

[OPEN] Contract expiry/renewal length — client confirmed the contract WILL expire after a certain period but the duration is "will tell you later"
...
[OPEN] Hotel-creation permission — Admin only, or can Regional Manager add hotels to their own group?
...
✅ QES / Skribble integration — new, being created
...
OPEN ITEMS (the only 3 unresolved product questions)

[OPEN] Probation legal shape — fixed-term trial contract vs. permanent contract with probation clause. Determines whether QES is legally mandatory from day one. (Client: "to be answered later.")

[OPEN] Contract expiry length — confirmed the contract expires, but the duration is pending. (Client: "will tell you.")
